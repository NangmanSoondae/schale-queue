...
class QueueIntegrationTest {
    ...
    void enqueue_assigns_fifo_positions_by_timestamp() {
        ...
        QueueService queue = new QueueService(redis, clock);
    ...
    void concurrent_enqueue_assigns_unique_positions() throws InterruptedException {
        ...
        QueueService queue = new QueueService(redis, Clock.systemUTC());
        int memberCount = 100;
    ...
    void admission_token_expires_after_ttl() throws InterruptedException {
        // given — TTL 300ms 짜리 토큰
        QueueProperties props = new QueueProperties();
        props.setAdmissionTtl(Duration.ofMillis(300));
        AdmissionTokenService tokens = new AdmissionTokenService(redis, props);
        ...
        // then — TTL 경과 후 자동 만료되어 무효
        Thread.sleep(500);
        assertThat(tokens.hasValidToken(GOODS, 1L))
