...
**핵심(우선 구현)** — 이 프로젝트의 본령:
- 대기열·재고 예약·주문·결제(선착순 코어), 동시성 제어 (Phase 2~3)

**정의 완료 · 구현 후순위** — 정책/설계는 확정, 구현은 핵심 이후:
- **상품 유형 분리**(예매형/배송형, ADR-005) 및 **환불·취소·이행**(ADR-006): 예매 시간차 부분환불, 배송 구매확정/반품, 취소표 쿨다운, 보상환불
- 배송/물류 이행 생명주기(배송완료→구매확정), 정산 상세(이벤트 훅만 마련)

**범위 밖(현 시점)**:
- 실제 PG사 상용 연동(모의/샌드박스로 대체), 쿠폰·프로모션 엔진, 다국어/통화 다변화, 지정좌석(예매형 좌석 단위)
