...
## [No.16] api 재시작 후 nginx 502 지속 — proxy_pass 정적 호스트명의 기동 시 1회 DNS 고정

- **일자**: 2026-07-06
- **관련 Phase**: Phase 5 이후 (리뷰2 수정 검증)
- **태그**: `#nginx` `#Docker` `#SSE` `#프록시`

### 1) 발견 (Discovery)
> 리뷰2 M-8(SSE 오류 재연결) E2E 검증 중, api 컨테이너를 재시작해 순단을 재현했더니 api 가 healthy 로 복귀한 뒤에도 프론트가 영원히 "재시도 중"에 머물렀다.

```
schale-api   Up About a minute (healthy)
health=200          # 호스트 → api 직접
via-nginx=502       # 호스트 → nginx(:3000) → api  ← api 정상인데 502 지속
```

`docker compose restart frontend`(nginx 재기동) 즉시 200 으로 회복 — 원인이 앱이 아니라 nginx 임을 특정.

### 2) 분석 (Analysis)
nginx 는 `proxy_pass http://api:8080;` 처럼 **정적 호스트명**을 쓰면 설정 로드 시 DNS 를 1회 해석해 IP 를 고정한다. Docker 네트워크에서 api 컨테이너가 재시작하면 다른 IP 를 받을 수 있고, nginx 는 죽은 이전 IP 로 계속 프록시 → upstream connect 실패 → 502. 프론트의 재연결 백오프(방금 넣은 M-8)도 이 앞에서는 무력하다 — 클라이언트가 아무리 재시도해도 프록시가 죽은 곳을 가리킨다.

### 3) 고찰 (Contemplation)
- **방안 A — Docker 내장 DNS 리졸버 + 변수 proxy_pass**: `resolver 127.0.0.11 valid=10s` + `set $api_upstream ...; proxy_pass $api_upstream;` — 변수 사용 시 nginx 가 요청 시점에 재해석(TTL 10s). 설정 2줄, 재기동 불요. 주의: 변수 proxy_pass 는 URI 전달 규칙이 달라지지만, URI 미부착 형태라 기존 시맨틱과 동일.
- **방안 B — upstream 블록 + resolve 파라미터**: 상용(nginx plus) 전용 기능이라 불가.
- **방안 C — depends_on/기동 순서로 회피**: 순서는 최초 기동만 해결, '운영 중 재시작'은 못 막는다.

### 4) 해결 (Resolution)
방안 A 적용(frontend/nginx.conf). 검증: api 재시작 → **nginx 무재시작으로** `via-nginx=200` 자동 회복(구 구성 502 지속과 대조), SSE 재연결도 프록시 회복 즉시 정상 순번 복귀.

### 5) 결과 (Result)
컨테이너 재시작 내성이 프록시 계층까지 확보됐다. 교훈: **컨테이너 환경의 nginx proxy_pass 는 '호스트명을 썼다'고 동적인 게 아니다 — 변수로 강제해야 요청 시점 해석이 된다.** 그리고 재연결·재시도 로직의 E2E 검증은 반드시 '의존 서비스 실제 재시작'으로 해야 이런 경로 결함이 드러난다(단위 테스트로는 절대 안 잡힘).
