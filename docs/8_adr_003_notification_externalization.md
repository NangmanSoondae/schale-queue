...
**채널 범위(target scope)**: 게이트웨이는 단일 Discord 발송이 아니라 **멀티채널 발송 위임처**로 설계한다 — 호출자가 보낼 내용을 주면 게이트웨이가 채널로 **발송만** 한다. 단계적 도입을 권장한다.

| 채널 | 우선순위 | 내용 형태 | 비고 |
| :--- | :--- | :--- | :--- |
| Discord | 1차 (현행 알림 이관) | 자유 텍스트 | Claude 작업 알림 호환 |
| 이메일 | 1~2차 | 자유 텍스트 | SMTP/SES 등 |
| 앱 푸시(FCM/APNs) | 2차 | 자유 텍스트 | 디바이스 토큰은 **앱이 보유** |
| SMS / 알림톡(카카오) | **선택·추후** | 알림톡은 **템플릿형** | 개인 프로젝트라 미도입 가능. 도입 시 카카오 승인 템플릿 1개로 테스트만 |

---

## 3. 기대 계약 (Integration Contract) — 교체 후 호출자가 의존할 표면

> ⚠️ 아래는 호출자(schale-queue 등)가 **기대하는** 인터페이스다. **확정본은 `notify-gateway` 레포에서 정의**하며, 불일치 시 그쪽이 우선한다. (계약 SSOT = 게이트웨이 레포)

**원칙: 발송 위임 + 내용은 호출자 제공.** 호출자가 "어떤 채널로 · 누구에게 · 무슨 내용"을 주면 게이트웨이는 **발송만** 한다.

자유 텍스트형 채널 (Discord · 이메일 · 푸시):
...
{
  "channel": "discord",            // discord | email | push | ...
  "to":      "schale-ops",         // 논리 채널명 / 이메일 / 디바이스 토큰 (채널별)
  "title":   "[Claude Code 알림]",
  "message": "작업 완료: ..."        // 이모지 가능 (앱 코드 발송 → 셸 우회로 안전)
}
```
...
```jsonc
{ "channel": "kakao", "to": "<phone>", "template_id": "order_complete", "vars": { /* ... */ } }
```

- **채널 자격증명·매핑은 게이트웨이가 보유** → 호출자는 실제 웹훅 URL/벤더 키를 몰라도 된다. (시크릿 중앙화)
- **수신자(`to`)·채널 선택은 호출자(앱)가 결정**해 전달한다. 게이트웨이는 특정 서비스 회원을 모른다. (→ §5.3 경계)
- 인증은 **프로젝트별 API Key**.
...
- **수신자 결정·채널 선택·사용자 알림 설정은 앱(호출자) 도메인**이며 게이트웨이로 옮기지 않는다. (예: "회원+앱ID면 푸시 디폴트, 결제 시 이메일 선택" 판단은 schale-queue가 보유) → 게이트웨이를 **회원 비종속**으로 유지해 재사용성 확보.
- 알림톡 등 **템플릿형 채널은 선택·추후** 과제. 개인 프로젝트 범위에선 미도입 가능하며, 도입 시 카카오 승인 템플릿 기반 테스트로 한정한다.
